...
受影响的 W11-1 测试预期翻转（**翻转集 = 所有被 6.5 新行捕获的 GRAY 断言**，
实施中枚举，2026-09-08 二次裁定补全为 T7/T9/T12/T19/T21）：
- T7 "- 好的没问题"：GRAY → DIRECT（寒暄应答，仅因前缀 "- " 未被寒暄行捕获）
- T9 "先看下这个，然后再告诉我"：GRAY → DIRECT（设计讨论中承诺修复的旧规则误触发典型）
- T12 "Then we left."：GRAY → DIRECT（英文叙事短句）
- T19 "你好，帮我看看今天日程"：GRAY → DIRECT（单一祈使，寒暄仅为前缀）
- T21 "跑一下测试"：GRAY → DIRECT（与已批准流程图示例卡一致）
- 其余 GRAY 断言不变（判定依据是**是否被 6.5 行捕获**：T16 动词门、
  T20 长调查文本 ≥24 有效字符等仍为 GRAY；勘误初版"仅 T19/T21"的枚举
  为起草遗漏，以本款通则为准）
