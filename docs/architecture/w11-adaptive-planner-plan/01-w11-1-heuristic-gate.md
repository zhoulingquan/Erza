...
> L2 接线在 W11-2。
>
> **勘误注记（2026-09-08）**：本任务书 3.3 决策表遗漏了已批准流程图 STEP 3
> 的"单步查看/执行 → DIRECT"桶（短 + 无产出动词 + 无结构信号），导致部分
> GRAY 预期（翻转集 T7/T9/T12/T19/T21）与设计意图相悖。该行已由 02 任务书
> 勘误 E1.1 授权在 W11-2 批次补入（翻转集随之更新，以 E1.1 通则为准），
> 本文件不再单独修订。
