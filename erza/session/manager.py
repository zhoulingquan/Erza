...
from erza.config.paths import get_legacy_sessions_dir
...
from erza.utils.subagent_channel_display import scrub_subagent_announce_body

if TYPE_CHECKING:
    from erza.session.writes import SessionWriteService
...
class SessionManager:
    ...
    # 通过 ERZA_SESSION_CACHE_SIZE 环境变量可调整；<=0 表示无上限（向后兼容）。
    ...
    def __init__(self, workspace: Path, *, cache_max: int | None = None):
        ...
        if cache_max is None:
            try:
                env_val = int(
                    os.environ.get("ERZA_SESSION_CACHE_SIZE", str(self._DEFAULT_CACHE_MAX))
                )
            except ValueError:
    ...
    def _get_legacy_session_path(self, key: str) -> Path:
        """Legacy global session path (~/.erza/sessions/), 旧命名规则。"""
    ...
    def _load(self, key: str) -> Session | None:
        ...
        3. 全局 legacy 目录(``~/.erza/sessions/<safe_key_legacy>.jsonl``),
           原子 rename 迁移到 v2。
