"""OpenAI-compatible HTTP API server for a fixed Erza session.

Provides /v1/chat/completions and /v1/models endpoints.
All requests route to a single persistent API session.

This module belongs to the optional ``api`` extra (imports ``aiohttp``).
The only supported entry point is ``erza serve``, which imports it
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    model_name: str = request.app.get("model_name", "Erza")
...
async def handle_models(request: web.Request) -> web.Response:
    """GET /v1/models"""
    model_name = request.app.get("model_name", "Erza")
    return web.json_response(
        {
            "object": "list",
            "data": [
