...
def current_time_str(timezone: str | None = None) -> str:
    """Return the current time string using the system timezone."""
    now = datetime.now().astimezone()
    offset = now.strftime("%z")
    offset_fmt = f"{offset[:3]}:{offset[3:]}" if len(offset) == 5 else offset
    tz_name = now.tzname() or (time.strftime("%Z") or "UTC")
    return f"{now.strftime('%Y-%m-%d %H:%M (%A)')} ({tz_name}, UTC{offset_fmt})"


_UNSAFE_CHARS = re.compile(r'[<>:"/\\|?*]')
_TOOL_RESULT_PREVIEW_CHARS = 1200
_TOOL_RESULTS_DIR = ".erza/tool-results"
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the Erza package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("erza") / "templates" / template_name
        if tpl.is_file():
